...
async function runXboxOnboarding(){
  if (!ONBOARDING_ENABLED || window.State.settings.onboardingComplete) return;
  const root=$('#xbox-onboarding'), content=$('#onboard-content');
  if(!root||!content) return;
  root.hidden=false;

  const wait=ms=>new Promise(r=>setTimeout(r,ms));
  const show=(html,screen)=>{
  ...
  /* The reference opens the keyboard on an empty field with just a caret, so
     nothing is prefilled; an empty entry still falls back to Xboxtest below. */
  const initial=window.State.data.gamertag==='NewSasquatch'?'':window.State.data.gamertag;
  show(`
    <div class="onboard-panel gamertag-panel keyboard-state">
      <h1>Choose your Gamertag</h1>
      <p>This is how you'll be known on Xbox.</p>
  ...
  show(`
    <div class="onboard-panel color-panel">
      ...
      <div class="color-grid">
        ...
        <button data-color="#68d8d8" style="--sw:#68d8d8"></button>
        <button data-color="#f36b32" style="--sw:#f36b32"></button>
        <button data-color="#f6e45a" style="--sw:#f6e45a"></button>
        <button data-color="#a9bfff" style="--sw:#a9bfff"></button>
        <button data-color="#ec91db" style="--sw:#ec91db"></button>
        <button data-color="#c3efea" style="--sw:#c3efea"></button>
        <button data-color="#f6f6f6" style="--sw:#f6f6f6"></button>
...
async function boot(){
  ...
  setTimeout(() => {
    ...
    node.innerHTML = '<div class="version-snapshot-mark"><img src="assets/pwa/xbox-logo.svg" alt="" aria-hidden="true"></div><div class="version-snapshot-copy"><strong>Xbox Version 1.5</strong><span>Snapshot p4k8vx</span></div>';
    $('#toasts').append(node);
    requestAnimationFrame(() => node.classList.add('show'));
    setTimeout(() => {
