/* Final Stratus transport pass.
   Xbox browser -> Xbox /api/stratus -> Stratus API.
   Keep the whole session on the same Xbox backend route. */
(() => {
'use strict';
...
function statusText(msg, game){
  switch (msg?.status){
    ...
    case 'queue': return msg.queue_pos != null ? `Queue ${msg.queue_pos}` : 'In queue…';
